...
public class RedBlackBSTMap<Key extends Comparable<Key>, Value> implements MAP<Key, Value> {
    ...
    public static void main(String[] args) {
        if (args.length == 0) {
            StdOut.println("usage: javac RedBlackBSTMap <file>");
            return;
        }

        In in = new In(args[0]);
        RedBlackBSTMap<Integer, String> tree = new RedBlackBSTMap<>();
        while (!in.isEmpty()) {
            int key = in.readInt();
